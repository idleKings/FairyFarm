...
function refreshData(){
    ...
    EGGS_TO_HATCH_1FAIRY(function(eggs){
        eggstohatch1=eggs
    });
    getMyEggs(function(eggs){
        ...
        secondsuntilfull=eggstohatch1-eggs/lastNumFairy
        console.log('secondsuntilfull ',secondsuntilfull,eggstohatch1,eggs,lastNumFairy)
    ...
    getMyFairy(function(fairy){
        lastNumFairy=fairy
        var gfsdoc=document.getElementById('getfreefairy')
        if(fairy>0){
        ...
        var allnumfairy=document.getElementsByClassName('numfairy')
        for(var i=0;i<allnumfairy.length;i++){
            if(allnumfairy[i]){
                allnumfairy[i].textContent=translateQuantity(fairy,0)
            }
        }
        var productiondoc=document.getElementById('production')
        productiondoc.textContent=formatEggs(lastNumFairy*60*60)
    });
    updateBuyPrice()
    updateSellPrice()
	updateFairymasterPrice()
	updateCurrentFairymaster()
...
function updateEggNumber(eggs){
    var hatchfairyquantitydoc=document.getElementById('hatchfairyquantity')
    hatchfairyquantitydoc.textContent=translateQuantity(eggs,0)
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumFairy>0 && eggstohatch1>0){
        currentTime=new Date().getTime()
        if(currentTime/1000-lastHatchTime>eggstohatch1){
            return;
        }
        difference=(currentTime-lastUpdate)/1000
        additionalEggs=Math.floor(difference*lastNumFairy)
...
function updateFairymasterPrice(){
    var fairymasterpricedoc=document.getElementById('fairymasterprice')
	getFairymasterReq(function(req) {
		fairymasterpricedoc.textContent = translateQuantity(req, 0);
	});
}


function updateCurrentFairymaster(){
    var currentfairymasterdoc=document.getElementById('currentfairymaster')
    //eggstobuydoc.textContent='?'
	currentfairymaster.textContent=ceoAddress()
}

function getFreeFairy2(){
    var ethtospenddoc=0.001//document.getElementById('freefairyspend')
    weitospend=web3.toWei(ethtospenddoc,'ether')
    getFreeFairy(weitospend,function(){
...
function disableButtons(){
    var allnumfairy=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumfairy.length;i++){
        if(allnumfairy[i]){
            allnumfairy[i].style.display="none"
        }
    }
}
function enableButtons(){
    ...
    for(var i=0;i<allnumfairy.length;i++){
        if(allnumfairy[i]){
            allnumfairy[i].style.display="inline-block"
        }
    }
}
